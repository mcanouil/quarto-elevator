...
# The extension registers its bundled arrival sound with
# `add_format_resource`, which writes it into the project directory while a
# page renders. That is after Quarto has resolved the project resources, so on
# a clean checkout the file never reaches the output and the ding 404s. Copying
# it here, before the render, lets the `resources` entry in _quarto.yml find
# it.
BUNDLED_DING="${ROOT_DIR}/_extensions/elevator/ding.mp3"
if [[ -f "${BUNDLED_DING}" ]]; then
	cp -f "${BUNDLED_DING}" "${DOCS_DIR}/ding.mp3"
	printf '[pre-render] ding.mp3 -> docs/ding.mp3\n'
else
	printf '[pre-render] No bundled ding.mp3; the arrival sound will not play\n' >&2
fi
